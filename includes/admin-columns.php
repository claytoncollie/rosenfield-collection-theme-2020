...
function setup(): void {
	...
	add_action( 'manage_posts_columns', __NAMESPACE__ . '\post_column_titles' );
	...
	add_action( 'manage_users_custom_column', __NAMESPACE__ . '\user_column_content', 10, 3 );
 ...
 */
function post_column_content( string $column_name, string $post_id ): void {
	...
	if ( OBJECT_PRICE === $column_name ) {
		...
		if ( $purchase_price ) {
			echo esc_html( (string) $purchase_price );
		}
	}
	...
	if ( OBJECT_DATE === $column_name ) {
		...
		if ( $purchase_date ) {
			echo esc_html( (string) $purchase_date );
		}
	}
}
 ...
 */
function user_column_content( string $value, string $column_name, string $user_id ): string {
	if ( 'letter' === $column_name ) {
		...
		return sprintf(
			...
			esc_html( ucwords( (string) $letter ) )
		);
	}
 ...
 */
function form_taxonomy_column_content( string $content, string $column_name, string $term_id ): string {
	if ( OBJECT_PREFIX !== $column_name ) {
		return $content;
	}
